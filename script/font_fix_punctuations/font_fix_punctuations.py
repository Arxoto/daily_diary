...
NEW_URL = "https://wom.cc"  # 个人主页或项目主页
NEW_VERSION = "1.000"  # 版本号

# 自定义字体家族名称
NEW_FAMILY_NAME = "WomPunctuationFixed1200"
...
def process_font(file_path):
    ...
    # 保留需要的字符，删除其他所有字符
    keep_codes = list(TARGETS_FULL.keys())
    ...
    for code in TARGETS_FULL:
        if code in font:
            glyph = font[code]
            diff = int(FULL_WIDTH - glyph.width)
            if diff == 0:
                break
            if "left" in TARGETS_FULL[code]:
                # glyph.left_side_bearing += diff
                glyph.transform(psMat.translate(diff, 0))
            elif "right" in TARGETS_FULL[code]:
                pass
            else:
                # glyph.left_side_bearing += diff / 2
                glyph.transform(psMat.translate(int(diff / 2), 0))
